 ...
 * */
	...
	bool val = 1;
	if(socketopt(sockfd, IPPROTO_IP, IP_MULTICAST_LOOP, &val, sizeof(val)) < 0)
	{
	...
	//父进程从网络上收包发给子进程
	//收节目单
	msg_list_st *msg_list = (msg_list_st*)malloc(MSG_LIST_MAX);
	if(mag_list == NULL)
	{
	...
	{
		int packlen = 0;
		if (( packlen = recvform(sockfd, msg_list, MSG_LIST_MAX, 0
		       (struct sockaddr*)&serveraddr, &serveraddrlen) ) < sizeof(msg_list_St))
		{
			fprintf(stderr, "message is too small.\n");
			continue;
		}
		if(serveraddr.chnid != LISTCHNID )
		{
	...
	//打印节目单
	msg_list_st *pos = msg_list->entry;
	for(; (char*)pos < ((char*)msg_list + len) ; 
			pos+= (msg_list_st*)(((char*)pos) + ntohs(pos->len)))
	...
	while(1)
	{
		...
		if(raddr.sin_port != serveraddr.sin_port || raddr.sin_addr != serveraddr.sin_addr)
		{
	...
	if(setsockopt(sockfd, IPPROTO_IP, IP_DROP_MEMBETSHIP, &mreq, sizeof(mreq)) < )
	{
		perror("setsockopt():");
		exit(1);
	}
